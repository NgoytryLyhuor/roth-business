...
export const ScrubInvoicePreview: React.FC<InvoicePreviewProps> = ({ invoice, onBack }) => {
  ...
  const handleDownload = async () => {
    ...
    link.download = `Scrub_Invoice_${invoice.customerName}_${invoice.date.replace(/-/g, '')}.png`;
  ...
  const handleShare = async () => {
    ...
    canvas.toBlob(async (blob) => {
      ...
      if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
        try {
          await navigator.share({ files: [file], title: `Scrub Invoice - ${invoice.customerName}` });
        } catch (error: unknown) {
          if (error instanceof Error && error.name !== 'AbortError') {
            window.open(`https://t.me/share/url?text=${encodeURIComponent(`Scrub Invoice for ${invoice.customerName}\nTotal: ${formatCurrency(invoice.total, invoice.currency)}`)}`, '_blank');
          }
        }
      } else {
        window.open(`https://t.me/share/url?text=${encodeURIComponent(`Scrub Invoice for ${invoice.customerName}\nTotal: ${formatCurrency(invoice.total, invoice.currency)}`)}`, '_blank');
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-3">
      <div className="max-w-2xl mx-auto mb-3 flex gap-2">
        ...
        <button onClick={handleDownload} className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded text-sm font-semibold hover:bg-emerald-700 disabled:opacity-50" disabled={isProcessing}>
